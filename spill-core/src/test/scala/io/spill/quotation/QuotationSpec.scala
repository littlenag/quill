...
class QuotationSpec extends Spec {
  ...
  "quotes and unquotes asts" - {
    ...
    "query" - {
      "schema" - {
        ...
        "with property alias" in {
          ...
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            "SomeAlias",
            List(
              PropertyAlias(List("s"), "theS"),
              PropertyAlias(List("i"), "theI")
            ),
            Fixed
          )
        }
        "with embedded property alias" in {
          ...
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            "SomeAlias",
            List(
              PropertyAlias(List("ev", "s"), "theS"),
              PropertyAlias(List("ev", "i"), "theI")
            ),
            Fixed
          )
        }
        ...
        "explicit `Predef.ArrowAssoc`" in {
          val q = quote {
            querySchema[TestEntity](
              "TestEntity",
              e => Predef.ArrowAssoc(e.s).->[String]("theS")
            )
          }
          quote(unquote(q)).ast mustEqual Entity.Opinionated(
            "TestEntity",
        ...
        "with only some properties renamed" in {
          val q = quote {
            querySchema[TestEntity]("SomeAlias", _.s -> "theS")
              .filter(t => t.s == "s" && t.i == 1)
          }
          quote(unquote(q)).ast mustEqual (
            Filter(
              ...
              Ident("t"),
              (Property(Ident("t"), "s") +==+ Constant("s")) +&&+ (Property(
                Ident("t"),
                "i"
              ) +==+ Constant(1))
            )
      ...
      "take" in {
        ...
        quote(unquote(q)).ast mustEqual Take(
          Entity("TestEntity", Nil),
          Constant(10)
        )
      }
      "drop" in {
        ...
        quote(unquote(q)).ast mustEqual Drop(
          Entity("TestEntity", Nil),
          Constant(10)
        )
      }
    ...
    "action" - {
      "update" - {
        "field" in {
          ...
          quote(unquote(q)).ast mustEqual Update(
            Entity("TestEntity", Nil),
            List(
              Assignment(Ident("t"), Property(Ident("t"), "s"), Constant("s"))
            )
          )
        }
      ...
      "insert" - {
        ...
        "batch" in {
          ...
          quote(unquote(q)).ast mustEqual
            Foreach(
              ScalarQueryLift("q.list", list, intEncoder),
              Ident("i"),
              delete.ast.body
            )
        }
        "batch with Quoted[Action[T]]" in {
          ...
          quote(unquote(q)).ast mustEqual
            Foreach(
              CaseClassQueryLift("q.list", list),
              Ident("row"),
              insert.ast.body
            )
        }
      ...
      "tuple" - {
        ...
        "arrow assoc" - {
          ...
          "normal arrow" in {
            val q = quote(1 -> "a" -> "b")
            quote(unquote(q)).ast mustEqual Tuple(
              List(Tuple(List(Constant(1), Constant("a"))), Constant("b"))
            )
          }
          "explicit `Predef.ArrowAssoc`" in {
    ...
    "function apply" - {
      ...
      "function reference" in {
        ...
        quote(unquote(q)).ast.body mustEqual FunctionApply(
          Ident("f"),
          List(Constant("a"))
        )
      }
      ...
      "==" - {
        ...
        "comparing compatible nested types" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual (OptionIsEmpty(ia) +&&+ OptionIsEmpty(
              ib
            )) +||+ (OptionIsDefined(ia) +&&+ OptionIsDefined(ib) +&&+ (ia +==+ ib))
          }
        ...
        "extras" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual OptionIsDefined(ia) +&&+ OptionIsDefined(
              ib
            ) +&&+ (ia +==+ ib)
          }
          "succeeds when Option/T" in {
      ...
      "!=" - {
        ...
        "comparing compatible nested types" - {
          ...
          "succeeds when Option/Option" in {
            ...
            quote(unquote(q)).ast.body mustEqual (OptionIsDefined(ia) +&&+ OptionIsEmpty(
              ib
            )) +||+ (OptionIsEmpty(ia) +&&+ OptionIsDefined(ib)) +||+ (ia +!=+ ib)
          }
    ...
    "unary operation" - {
      "-" in {
        ...
        quote(unquote(q)).ast.body mustEqual UnaryOperation(
          NumericOperator.`-`,
          Ident("a")
        )
      }
    ...
    "infix" - {
      ...
      "with params" in {
        ...
        quote(unquote(q)).ast.body mustEqual Infix(
          List("", " || ", ""),
    ...
    "option operation" - {
      ...
      "map" - {
        "simple" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionMap(
            Ident("o"),
            Ident("v"),
            Ident("v")
          )
        ...
        "unchecked" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionTableMap(
            Ident("o"),
            Ident("v"),
            Ident("v")
          )
      ...
      "flatMap" - {
        ...
        "unchecked" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionTableFlatMap(
            Ident("o"),
            Ident("v"),
            OptionApply(Ident("v"))
          )
      ...
      "getOrElse" in {
        ...
        quote(unquote(q)).ast.body mustEqual OptionGetOrElse(
          Ident("o"),
          Constant(11)
        )
      }
        ...
        quote(unquote(q)).ast.body mustEqual
          OptionGetOrElse(
            OptionMap(
              Ident("o"),
              Ident("i"),
              BinaryOperation(Ident("i"), NumericOperator.`<`, Constant(10))
            ),
            Constant(true)
          )
      }
      ...
      "forall" - {
        "simple" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionForall(
            Ident("o"),
            Ident("v"),
            Ident("v")
          )
      ...
      "exists" - {
        "simple" in {
          ...
          quote(unquote(q)).ast.body mustEqual OptionExists(
            Ident("o"),
            Ident("v"),
            Ident("v")
          )
      ...
      "contains" in {
        ...
        quote(unquote(q)).ast.body mustEqual OptionContains(
          Ident("o"),
          Ident("v")
        )
      }
    ...
    "traversable operations" - {
      ...
      "list.contains" in {
        ...
        quote(unquote(q)).ast.body mustEqual ListContains(
          Ident("l"),
          Ident("v")
        )
      }
    ...
    "dynamic" - {
      ...
      "quoted dynamic" in {
        ...
        quote(unquote(q)).ast mustEqual BinaryOperation(
          Constant(1),
          NumericOperator.`+`,
          Constant(1)
        )
    ...
    "if" - {
      ...
      "nested" in {
        ...
        quote(unquote(q)).ast.body mustEqual If(
          Ident("c1"),
          Constant(1),
          If(Ident("c2"), Constant(2), Constant(3))
        )
      }
    }
    "ord" in {
    ...
    "aggregates liftings of nested quotations" - {
      ...
      "with implicit class" - {
        "constant" in {
          ...
          val l =
            q.liftings.`implicits.PlusRadom(null.asInstanceOf[Int(1)]).plusRandom.Implicits.this.random`
  ...
  "unquotes referenced quotations" in {
    val q = quote(1)
    val q2 = quote(q + 1)
    quote(unquote(q2)).ast mustEqual BinaryOperation(
      Constant(1),
      NumericOperator.`+`,
      Constant(1)
    )
