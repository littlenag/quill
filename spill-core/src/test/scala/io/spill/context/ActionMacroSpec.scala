...
class ActionMacroSpec extends Spec {
  ...
  "runs non-batched action" - {
    ...
    "insert returning" - {
      ...
      "returning value - with single - should not compile" in testContext
        .withDialect(MirrorIdiomReturningSingle) { ctx =>
          "import ctx._; ctx.run(qr1.insert(t => t.i -> 1).returning(t => t.l))" mustNot compile
        }
      "returning value - with multi" in testContext.withDialect(
        MirrorIdiomReturningMulti
      ) { ctx =>
        import ctx._
        val q = quote {
      ...
      "returning generated value - with single" in testContext.withDialect(
        MirrorIdiomReturningSingle
      ) { ctx =>
        import ctx._
        val q = quote {
      ...
      "returning generated value - with single - multi should not compile" in testContext
        .withDialect(MirrorIdiomReturningSingle) { ctx =>
          "import ctx._; ctx.run(qr1.insert(t => t.i -> 1).returningGenerated(t => (t.l, t.i))" mustNot compile
        }
      "returning generated value - with multi" in testContext.withDialect(
        MirrorIdiomReturningMulti
      ) { ctx =>
        import ctx._
        val q = quote {
      ...
      "returning generated value - with multi - operation in clause should not compile" in testContext
      ...
      "returning generated value - with multi - single" in testContext
        .withDialect(MirrorIdiomReturningMulti) { ctx =>
          import ctx._
          val q = quote {
            qr1.insert(t => t.i -> 1).returningGenerated(t => t.l)
      ...
      "case class lifting + returning generated value" in {
        val q = quote {
          qr1
            .insert(lift(TestEntity("s", 1, 2L, None)))
            .returningGenerated(t => t.l)
        }
        val r = testContext.run(q)
    ...
    "update returning" - {
      ...
      "case class lifting + returning multi value" in {
        val q = quote {
          qr1
            .update(lift(TestEntity("s", 1, 2L, None)))
            .returning(t => (t.l, t.i))
        }
        val r = testContext.run(q)
  ...
  "runs batched action" - {
    val entities =
      List(TestEntity("s1", 2, 3L, Some(4)), TestEntity("s5", 6, 7L, Some(8)))
    ...
    "scalar" in {
      ...
      r.groups mustEqual List(
        """querySchema("TestEntity").insert(t => t.i -> ?)""" -> List(
          Row(1),
          Row(2)
        )
      )
    ...
    "case class + returning generated" in {
      val q = quote {
        liftQuery(entities).foreach(
          p => qr1.insert(p).returningGenerated(t => t.l)
        )
      }
      val r = testContext.run(q)
      r.groups mustEqual List(
    ...
    "case class + returning generated + nested action" in {
      val insert = quote { (p: TestEntity) =>
        qr1.insert(p).returningGenerated(t => t.l)
      }
      val r = testContext.run(liftQuery(entities).foreach(p => insert(p)))
      r.groups mustEqual List(
        (
