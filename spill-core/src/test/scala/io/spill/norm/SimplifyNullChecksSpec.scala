...
class SimplifyNullChecksSpec extends Spec {
  ...
  "center rule must" - {
    ...
    "apply left rule" in {
      SimplifyNullChecksAnsi(IfExist(IfExistElseNull(ia, ib), ca, it)) mustEqual If(
        IsNotNullCheck(ia) +&&+ IsNotNullCheck(ib),
        ca,
        it
      )
    }
    ...
    "apply right rule" in {
      SimplifyNullChecksAnsi(IfExistElseNull(ia, IfExistElseNull(ib, it))) mustEqual If(
        IsNotNullCheck(ia) +&&+ IsNotNullCheck(ib),
        it,
        NullValue
      )
    ...
    "reduces when Option == Option(constant)" in {
      ...
      SimplifyNullChecksNonAnsi(quote(unquote(q)).ast.body) mustEqual (OptionIsDefined(
        ia
      ) +&&+ (ia +==+ OptionApply(Constant(1))))
    }
    ...
    "reduces when Option != Option(constant)" in {
      ...
      SimplifyNullChecksAnsi(quote(unquote(q)).ast.body) mustEqual (OptionIsEmpty(
        ia
      ) +||+ (ia +!=+ OptionApply(Constant(1))))
    }
    ...
    "with ansi enabled" - {
      "Ignores regular comparison - Int/Int" in {
        ...
        SimplifyNullChecksAnsi(quote(unquote(q)).ast.body) mustEqual BinaryOperation(
          Ident("a"),
          EqualityOperator.`==`,
          Ident("b")
        )
    ...
    "without ansi enabled" - {
      "Ignores regular comparison - Int/Int" in {
        ...
        SimplifyNullChecksNonAnsi(quote(unquote(q)).ast.body) mustEqual BinaryOperation(
          Ident("a"),
          EqualityOperator.`==`,
          Ident("b")
        )
