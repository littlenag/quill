...
class NormalizeReturningSpec extends Spec {
  ...
  "do not remove assignment if embedded has columns with the same name" - {
    ...
    "when returning parent col - single - returning should not compile" in testContext
      .withDialect(MirrorIdiomReturningSingle) { ctx =>
        "ctx.run(query[Entity].insert(lift(e)).returning(p => p.id))" mustNot compile
      }
    "when returning parent col - single - returning generated" in testContext
      .withDialect(MirrorIdiomReturningSingle) { ctx =>
        import ctx._
        val r =
    ...
    "when returning embedded col - single" in testContext.withDialect(
      MirrorIdiomReturningSingle
    ) { ctx =>
      import ctx._
      val r =
