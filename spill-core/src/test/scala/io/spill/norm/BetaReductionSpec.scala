...
class BetaReductionSpec extends Spec {
  ...
  "replaces identifiers by actuals" - {
    ...
    "with inline" - {
      ...
      "top level block" in {
        val block = Block(List(Val(a, entity), Val(b, a), Map(b, d, c1)))
        BetaReduction(block) mustEqual Map(entity, d, c1)
      }
      "nested blocks" in {
        val inner = Block(
          List(Val(a, entity), Val(b, c2), Val(c, c3), Tuple(List(a, b, c)))
        )
        val outer = Block(List(Val(a, inner), Val(b, a), Val(c, b), c))
      ...
      "nested blocks caseclass" in {
        ...
        val outer = Block(List(Val(a, inner), Val(b, a), Val(c, b), c))
        BetaReduction(outer) mustEqual CaseClass(
