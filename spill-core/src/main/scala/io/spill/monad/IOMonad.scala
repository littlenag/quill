...
trait IOMonad {
  ...
  object IO {
    ...
    def sequence[A, M[X] <: IterableOnce[X], E <: Effect](
      in: M[IO[A, E]]
    )(implicit cbfResultToValue: Factory[A, M[A]]): IO[M[A], E] =
      Sequence(in, cbfResultToValue)

    val unit: IO[Unit, Effect] = fromTry(Success(()))

    def zip[T, E1 <: Effect, S, E2 <: Effect](
      a: IO[T, E1],
      b: IO[S, E2]
    ): IO[(T, S), E1 with E2] =
    ...
    def failed[T](exception: Throwable): IO[T, Effect] =
    ...
    def foldLeft[T, R, E <: Effect](
      ios: collection.immutable.Iterable[IO[T, E]]
    )(zero: R)(op: (R, T) => R): IO[R, E] =
      sequence(ios).map(_.foldLeft(zero)(op))

    def reduceLeft[T, R >: T, E <: Effect](
      ios: collection.immutable.Iterable[IO[T, E]]
    )(op: (R, T) => R): IO[R, E] =
      sequence(ios).map(_.reduceLeft(op))

    def traverse[A, B, M[X] <: IterableOnce[X], E <: Effect](
      in: M[A]
    )(fn: A => IO[B, E])(implicit cbf: Factory[B, M[B]]): IO[M[B], E] =
  ...
  trait IO[+T, -E <: Effect] {
        ...
        case Success(_) =>
          Failure(
            new NoSuchElementException(
              "IO.failed not completed with a throwable."
            )
          )
      }
    ...
    def flatMap[S, E2 <: Effect](f: T => IO[S, E2]): IO[S, E with E2] =
      transformWith {
        case Success(s) => f(s)
        case f @ Failure(_) =>
          IO.fromTry[S](f.asInstanceOf[Failure[S]])
            .asInstanceOf[IO[S, E with E2]]
      }
      ...
      map { r =>
        if (p(r)) r
          ...
          throw new NoSuchElementException(
            "IO.filter predicate is not satisfied"
          )
      }
    ...
    def recoverWith[U >: T, E2 <: Effect](
      pf: PartialFunction[Throwable, IO[U, E2]]
    ): IO[U, E with E2] =
      transformWith {
