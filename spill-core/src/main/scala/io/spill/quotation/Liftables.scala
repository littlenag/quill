...
trait Liftables {
  ...
  implicit val optionOperationLiftable: Liftable[OptionOperation] =
    Liftable[OptionOperation] {
  ...
  implicit val traversableOperationLiftable: Liftable[IterableOperation] =
    Liftable[IterableOperation] {
  ...
  implicit val binaryOperatorLiftable: Liftable[BinaryOperator] =
    Liftable[BinaryOperator] {
  ...
  implicit val unaryOperatorLiftable: Liftable[UnaryOperator] =
    Liftable[UnaryOperator] {
      case NumericOperator.`-`          => q"$pack.NumericOperator.`-`"
      case BooleanOperator.`!`          => q"$pack.BooleanOperator.`!`"
  ...
  implicit val aggregationOperatorLiftable: Liftable[AggregationOperator] =
    Liftable[AggregationOperator] {
  ...
  implicit val queryLiftable: Liftable[Query] = Liftable[Query] {
    case Entity.Opinionated(a, b, renameable) =>
      q"$pack.Entity.Opinionated($a, $b, $renameable)"
  ...
  implicit val propertyAliasLiftable: Liftable[PropertyAlias] =
    Liftable[PropertyAlias] {
  ...
  implicit val conflictTargetLiftable: Liftable[OnConflict.Target] =
    Liftable[OnConflict.Target] {
      case OnConflict.NoTarget      => q"$pack.OnConflict.NoTarget"
      case OnConflict.Properties(a) => q"$pack.OnConflict.Properties.apply($a)"
    }

  implicit val conflictActionLiftable: Liftable[OnConflict.Action] =
    Liftable[OnConflict.Action] {
  ...
  implicit val externalIdentLiftable: Liftable[ExternalIdent] =
    Liftable[ExternalIdent] {
      case ExternalIdent(a) => q"$pack.ExternalIdent($a)"
    }

  implicit val liftLiftable: Liftable[Lift] = Liftable[Lift] {
    case ScalarValueLift(a, b: Tree, c: Tree) =>
      q"$pack.ScalarValueLift($a, $b, $c)"
    case CaseClassValueLift(a, b: Tree) => q"$pack.CaseClassValueLift($a, $b)"
    case ScalarQueryLift(a, b: Tree, c: Tree) =>
      q"$pack.ScalarQueryLift($a, $b, $c)"
