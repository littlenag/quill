...
trait Parsing extends ValueComputation {
  ...
  case class Parser[T](p: PartialFunction[Tree, T])(implicit ct: ClassTag[T]) {
    ...
    def apply(tree: Tree) =
      unapply(tree).getOrElse {
        c.fail(
          s"Tree '$tree' can't be parsed to '${ct.runtimeClass.getSimpleName}'"
        )
      }
  ...
  private def patMatchParser(
    ...
    def reductions(ast: Ast, path: List[Int] = List()): List[(Ident, Ast)] = {
      ast match {
        ...
        case other =>
          c.fail(
            s"Please report a bug. Expected tuple, val, or ident, got '$other'"
          )
      }
    }
  ...
  val liftParser: Parser[Lift] = Parser[Lift] {
    ...
    case q"$pack.liftScalar[$t]($value)($encoder)" =>
      ScalarValueLift(value.toString, value, encoder)
    case q"$pack.liftCaseClass[$t]($value)" =>
      CaseClassValueLift(value.toString, value)

    case q"$pack.liftQueryScalar[$t, $u]($value)($encoder)" =>
      ScalarQueryLift(value.toString, value, encoder)
    case q"$pack.liftQueryCaseClass[$t, $u]($value)" =>
      CaseClassQueryLift(value.toString, value)

    // Unused, it's here only to make eclipse's presentation compiler happy :(
    case q"$pack.lift[$t]($value)" =>
      ScalarValueLift(value.toString, value, q"null")
    case q"$pack.liftQuery[$t, $u]($value)" =>
      ScalarQueryLift(value.toString, value, q"null")
  }
  ...
  val queryParser: Parser[Ast] = Parser[Ast] {
    ...
    case q"$source.filter(($alias) => $body)" if (is[CoreDsl#Stream[Any]](source)) =>
      Filter(astParser(source), identParser(alias), astParser(body))

    case q"$source.withFilter(($alias) => $body)" if (is[CoreDsl#Stream[Any]](source)) =>
      Filter(astParser(source), identParser(alias), astParser(body))

    case q"$source.map[$t](($alias) => $body)" if (is[CoreDsl#Stream[Any]](source)) =>
      Map(astParser(source), identParser(alias), astParser(body))

    case q"$source.flatMap[$t](($alias) => $body)" if (is[CoreDsl#Stream[Any]](source)) =>
      FlatMap(astParser(source), identParser(alias), astParser(body))

    case q"$source.concatMap[$t, $u](($alias) => $body)($ev)" if (is[CoreDsl#Stream[Any]](source)) =>
      ConcatMap(astParser(source), identParser(alias), astParser(body))

    case q"$source.sortBy[$t](($alias) => $body)($ord)" if (is[CoreDsl#Stream[Any]](source)) =>
      SortBy(
        astParser(source),
        identParser(alias),
        astParser(body),
    ...
    case q"$source.groupBy[$t](($alias) => $body)" if (is[CoreDsl#Stream[Any]](source)) =>
      GroupBy(astParser(source), identParser(alias), astParser(body))

    case q"$a.value[$t]" if (is[CoreDsl#Stream[Any]](a)) => astParser(a)
    case q"$a.min[$t]" if (is[CoreDsl#Stream[Any]](a)) =>
      Aggregation(AggregationOperator.`min`, astParser(a))
    case q"$a.max[$t]" if (is[CoreDsl#Stream[Any]](a)) =>
      Aggregation(AggregationOperator.`max`, astParser(a))
    case q"$a.avg[$t]($n)" if (is[CoreDsl#Stream[Any]](a)) =>
      Aggregation(AggregationOperator.`avg`, astParser(a))
    case q"$a.sum[$t]($n)" if (is[CoreDsl#Stream[Any]](a)) =>
      Aggregation(AggregationOperator.`sum`, astParser(a))
    case q"$a.size" if (is[CoreDsl#Stream[Any]](a)) =>
      Aggregation(AggregationOperator.`size`, astParser(a))

    case q"$source.take($n)" if (is[CoreDsl#Stream[Any]](source)) =>
      Take(astParser(source), astParser(n))

    case q"$source.drop($n)" if (is[CoreDsl#Stream[Any]](source)) =>
      Drop(astParser(source), astParser(n))

    case q"$source.union[$t]($n)" if (is[CoreDsl#Stream[Any]](source)) =>
      Union(astParser(source), astParser(n))

    case q"$source.unionAll[$t]($n)" if (is[CoreDsl#Stream[Any]](source)) =>
      UnionAll(astParser(source), astParser(n))

    case q"$source.++[$t]($n)" if (is[CoreDsl#Stream[Any]](source)) =>
      UnionAll(astParser(source), astParser(n))
    ...
    case q"$source.distinct" if (is[CoreDsl#Stream[Any]](source)) =>
      Distinct(astParser(source))

    case q"$source.nested" if (is[CoreDsl#Stream[Any]](source)) =>
  ...
  implicit val orderingParser: Parser[Ordering] = Parser[Ordering] {
    ...
    case q"$pack.Ord.apply[..$t](..$elems)" =>
      TupleOrdering(elems.map(orderingParser(_)))
  ...
  val joinCallParser: Parser[(JoinType, Ast, Option[Ast])] =
    Parser[(JoinType, Ast, Option[Ast])] {
      case q"$a.join[$t, $u]($b)" if (is[CoreDsl#Stream[Any]](a)) =>
        (InnerJoin, astParser(a), Some(astParser(b)))
      case q"$a.leftJoin[$t, $u]($b)" if (is[CoreDsl#Stream[Any]](a)) =>
        (LeftJoin, astParser(a), Some(astParser(b)))
      case q"$a.rightJoin[$t, $u]($b)" if (is[CoreDsl#Stream[Any]](a)) =>
        (RightJoin, astParser(a), Some(astParser(b)))
      case q"$a.fullJoin[$t, $u]($b)" if (is[CoreDsl#Stream[Any]](a)) =>
        (FullJoin, astParser(a), Some(astParser(b)))

      case q"$a.join[$t]" if (is[CoreDsl#Stream[Any]](a)) =>
        (InnerJoin, astParser(a), None)
      case q"$a.leftJoin[$t]" if (is[CoreDsl#Stream[Any]](a)) =>
        (LeftJoin, astParser(a), None)
      case q"$a.rightJoin[$t]" if (is[CoreDsl#Stream[Any]](a)) =>
        (RightJoin, astParser(a), None)
    }
  ...
  private def identClean(x: Ident): Ident =
    x.copy(name = x.name.replace("$", ""))
  private def ident(x: TermName): Ident =
  ...
  val propertyParser: Parser[Property] = Parser[Property] {
    ...
    case q"$e.$property" =>
      ...
      if (caseAccessors.nonEmpty && !caseAccessors.contains(property))
        c.fail(
          s"Can't find case class property: ${property.decodedName.toString}"
        )
  ...
  case object Equal extends EqualityBehavior {
    def operator: BinaryOperator = EqualityOperator.`==`
  }
  ...
  private def equalityWithInnerTypechecksAnsi(left: Tree, right: Tree)(
    equalityBehavior: EqualityBehavior
  ) = {
    val (leftIsOptional, rightIsOptional) =
      checkInnerTypes(left, right, AllowInnerCompare)
    ...
    (leftIsOptional, rightIsOptional) match {
      case (true, true) =>
        OptionIsDefined(a) +&&+ OptionIsDefined(b) +&&+ comparison
  ...
  private def equalityWithInnerTypechecksIdiomatic(left: Tree, right: Tree)(
    equalityBehavior: EqualityBehavior
  ) = {
    val (leftIsOptional, rightIsOptional) =
      checkInnerTypes(left, right, ForbidInnerCompare)
    val a = astParser(left)
    val b = astParser(right)
    val comparison = BinaryOperation(a, equalityBehavior.operator, b)
    (leftIsOptional, rightIsOptional, equalityBehavior) match {
      ...
      case (true, true, Equal) =>
        (OptionIsEmpty(a) +&&+ OptionIsEmpty(b)) +||+ (OptionIsDefined(a) +&&+ OptionIsDefined(
          b
        ) +&&+ comparison)
      ...
      case (true, true, NotEqual) =>
        (OptionIsDefined(a) +&&+ OptionIsEmpty(b)) +||+ (OptionIsEmpty(a) +&&+ OptionIsDefined(
          b
        )) +||+ comparison
      ...
      case (lop, rop, _) => {
        ...
        c.abort(
          left.pos,
          s"Cannot compare ${lopString} with ${ropString} using operator ${equalityBehavior.operator}"
        );
      }
    }
  }
  ...
  val setOperationParser: Parser[Operation] = {
    val unary =
      operationParser(is[CoreDsl#Stream[Any]](_)) {
        case "isEmpty"  => SetOperator.`isEmpty`
        case "nonEmpty" => SetOperator.`nonEmpty`
      }
    Parser[Operation] {
      case q"$a.contains[$t]($b)" if (is[CoreDsl#Stream[Any]])(a) =>
  ...
  private def isTypeConstructor[T: WeakTypeTag] =
    c.weakTypeTag[T]
      .tpe != null && c.weakTypeTag[T].tpe.typeConstructor != NoType
  ...
  implicit class InsertReturnCapabilityExtension(
  ...
  val actionParser: Parser[Ast] = Parser[Ast] {
    ...
    case q"$query.foreach[$t1, $t2](($alias) => $body)($f)" if (is[CoreDsl#Stream[Any]](query)) =>
      // If there are actions inside the subtree, we need to do some additional sanitizations
      // of the variables so that their content will not collide with code that we have generated.
  ...
  private def reprocessReturnClause(
    ...
    (ident == originalBody, actionType.tpe) match {
      ...
      case (true, ClassTypeRefMatch(cls, List(arg))) if (cls == asClass[QueryDsl#Insert[_]] || cls == asClass[QueryDsl#Update[_]]) && isTypeCaseClass(arg) =>
        ...
        if (elements.size == 0)
          c.fail("Case class in the 'returning' clause has no values")
        ...
        val newBody =
          tpe match {
            ...
            case _ =>
              c.fail(
                "Could not process whole-record 'returning' clause. Consider trying to return individual columns."
              )
          }
        astParser(newBody)
  ...
  private val assignmentParser: Parser[Assignment] = Parser[Assignment] {
    ...
    // Unused, it's here only to make eclipse's presentation compiler happy
    case astParser(ast) =>
      Assignment(Ident("unused"), Ident("unused"), Constant("unused"))
  }
    ...
    optionCheckBehavior match {
      ...
      case _ =>
        if (leftIsOptional || rightIsOptional)
          ...
          c.abort(
            lhs.pos,
            s"${leftType.widen} == ${rightType.widen} is not allowed since they are different types."
          )
    }
  }
  ...
  private def typecheckUnquoted(tree: Tree): Tree = {
    ...
    try c.typecheck(unquoted(tree), c.TYPEmode)
    catch {
