...
trait ReifyLiftings {
  ...
  private case class ReifyLiftings(state: Map[TermName, Reified])
    ...
    private def reify(lift: Lift) =
      lift match {
        case ScalarValueLift(name, value: Tree, encoder: Tree) =>
          Reified(value, Some(encoder))
        case CaseClassValueLift(name, value: Tree) => Reified(value, None)
        case ScalarQueryLift(name, value: Tree, encoder: Tree) =>
          Reified(value, Some(encoder))
    ...
    private def unparse(ast: Ast): Tree =
      ast match {
        case Property(Ident(alias), name) =>
          q"${TermName(alias)}.${TermName(name)}"
    ...
    private def lift(v: Tree): Lift = {
      val tpe =
        c.typecheck(q"import _root_.scala.language.reflectiveCalls; $v").tpe
      OptionalTypecheck(c)(q"implicitly[${c.prefix}.Encoder[$tpe]]") match {
        ...
        case None =>
          tpe.baseType(c.symbolOf[Product]) match {
            case NoType =>
              c.fail(
                s"Can't find an encoder for the lifted case class property '$v'"
              )
    ...
    override def apply(ast: Ast) =
      ast match {
        ...
        case p: OptionTableFlatMap =>
          super.apply(p) match {
            case (p2 @ OptionTableFlatMap(_: CaseClassValueLift, _, _), _) =>
              apply(lift(unparse(p2)))
            case other => other
          }

        case p: OptionTableMap =>
          super.apply(p) match {
            case (p2 @ OptionTableMap(_: CaseClassValueLift, _, _), _) =>
              apply(lift(unparse(p2)))
            case other => other
          }
        ...
        case p: OptionFlatMap =>
          super.apply(p) match {
            case (p2 @ OptionFlatMap(_: CaseClassValueLift, _, _), _) =>
              apply(lift(unparse(p2)))
            case other => other
          }
        ...
        case p: OptionMap =>
          super.apply(p) match {
            case (p2 @ OptionMap(_: CaseClassValueLift, _, _), _) =>
              apply(lift(unparse(p2)))
            case other => other
          }
        ...
        case p: Property =>
          super.apply(p) match {
            case (p2 @ Property(_: CaseClassValueLift, _), _) =>
              apply(lift(unparse(p2)))
            case other => other
          }
