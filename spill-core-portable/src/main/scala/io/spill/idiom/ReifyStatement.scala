  ...
  private def token2string(
    ...
    def apply(
      ...
      case head :: tail =>
        head match {
          case StringToken(s2) =>
            apply(tail, s2 +: sqlResult, liftingResult, liftingSize)
          ...
          case ScalarLiftToken(lift) =>
            apply(
              tail,
              liftingPlaceholder(liftingSize) +: sqlResult,
              lift +: liftingResult,
              liftingSize + 1
            )
          ...
          case _: QuotationTagToken =>
            throw new UnsupportedOperationException(
              "Quotation Tags must be resolved before a reification."
            )
        }
    }
      ...
      statement.tokens.foldLeft(List.empty[Token]) {
        case (
          ...
          lift.value.asInstanceOf[Iterable[Any]].toList match {
            ...
            case values =>
              val liftings = values.map(
                v =>
                  ScalarLiftToken(ScalarValueLift(lift.name, v, lift.encoder))
              )
