...
object Messages {
  ...
  private[util] val traces: List[TraceType] =
    variable("quill.trace.types", "quill_trace_types", "standard")
      ...
      .map(_.trim)
      .flatMap(
        trace => TraceType.values.filter(traceType => trace == traceType.value)
      )
  ...
  object TraceType {
    ...
    def values: List[TraceType] =
      List(Standard, Normalizations, NestedQueryExpansion, AvoidAliasConflict)
  }
