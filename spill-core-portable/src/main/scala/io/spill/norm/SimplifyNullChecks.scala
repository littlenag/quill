...
import io.spill.norm.EqualityBehavior.AnsiEquality
...
class SimplifyNullChecks(equalityBehavior: EqualityBehavior)
  ...
  override def apply(ast: Ast): Ast =
    ast match {
      ...
      case OptionIsDefined(Optional(a)) +&&+ OptionIsDefined(Optional(b)) +&&+ (exp @ (Optional(
        a1
        ) `== or !=` Optional(b1))) if (a == a1 && b == b1 && equalityBehavior == AnsiEquality) =>
        apply(exp)

      case OptionIsDefined(Optional(a)) +&&+ (exp @ (Optional(a1) `== or !=` Optional(
        _
        ))) if (a == a1 && equalityBehavior == AnsiEquality) =>
        apply(exp)
      case OptionIsDefined(Optional(b)) +&&+ (exp @ (Optional(_) `== or !=` Optional(
        b1
