      ...
      case m @ Map(_, x, cc @ CaseClass(values)) if hasInfix(cc) => //Nested(m)
        Map(
          ...
          CaseClass(values.map {
            case (name, _) =>
              (name, Property(x, name)) // mappings of nested-query case class properties should not be renamed
          })
        )
      ...
      case m @ Map(_, x, tup @ Tuple(values)) if hasInfix(tup) =>
        Map(
          ...
          Tuple(values.zipWithIndex.map {
            case (_, i) =>
              Property(x, s"_${i + 1}") // mappings of nested-query tuple properties should not be renamed
          })
        )
