...
class AstPrinter(traceOpinions: Boolean, traceAstSimple: Boolean)
  ...
  override def additionalHandlers: PartialFunction[Any, Tree] = {
    ...
    case p: Property if (traceOpinions) =>
      Tree.Apply(
        ...
        List[Tree](
          treeify(p.ast),
          treeify(p.name),
          printRenameable(p.renameable),
          printVisibility(p.visibility)
        ).iterator
      )
    ...
    case e: Entity if (traceOpinions) =>
      Tree.Apply(
        ...
        List[Tree](
          treeify(e.name),
          treeify(e.properties),
          printRenameable(e.renameable)
        ).iterator
      )
  }
  ...
  def tokenize(x: Any): Iterator[fansi.Str] = {
    ...
    val renderer =
      new Renderer(defaultWidth, colorApplyPrefix, colorLiteral, defaultIndent)
